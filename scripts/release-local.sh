...
usage() {
  cat <<'EOF'
Generate local release artifacts (without tags or GitHub Release).

Usage:
  ./scripts/release-local.sh [options]

Options:
  --no-install          Do not run npm ci
  --no-build            Do not run npx tauri build
  --output-dir <dir>    Output base directory (default: local-artifacts/releases)
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  echo "Artifacts would be placed at: $RELEASE_DIR"
  exit 0
fi

if [[ ! -d "$BUNDLE_DIR" ]]; then
  echo "Artifacts directory not found: $BUNDLE_DIR" >&2
  echo "Run the script without --no-build or execute npx tauri build manually." >&2
  exit 1
fi
...
Contents:
- bundle/          Artifacts generated by Tauri, ready for manual upload
- SHA256SUMS.txt   Checksums (if shasum/sha256sum is available)
EOF

echo "Done."
echo "Local artifacts at: $RELEASE_DIR"
