...
const translateSelectionIconEl = document.getElementById("quick-translate-selection-icon");
const translateSelectionSpinnerEl = document.getElementById("quick-translate-selection-spinner");
const improveSelectionBtn = document.getElementById("quick-improve-selection-btn");
const improveSelectionIconEl = document.getElementById("quick-improve-selection-icon");
const improveSelectionSpinnerEl = document.getElementById("quick-improve-selection-spinner");
...
function setSelectionSpinner(visible) {
  if (translateSelectionIconEl) translateSelectionIconEl.hidden = visible;
  if (translateSelectionSpinnerEl) translateSelectionSpinnerEl.hidden = !visible;
}

function setImproveSpinner(visible) {
  if (improveSelectionIconEl) improveSelectionIconEl.hidden = visible;
  if (improveSelectionSpinnerEl) improveSelectionSpinnerEl.hidden = !visible;
}
...
async function runSelectionAction() {
  ...
  try {
    ...
    const output = await invoke("translate_text", { input, style: "simple" });
    ...
    setSelectionSpinner(false);
    setBusy(false);
    updateDictateButton();
  }
}

async function runImproveAction() {
  if (isBusy || isRecording) {
    return;
  }
  ...
  try {
    ...
    const output = await invoke("improve_text", { input, style: quickMode });
...
improveSelectionBtn.addEventListener("click", runImproveAction);
